...
def _build_cli_overrides(
    ...
    prompt_length_pct: Optional[float],
    eval_interval: Optional[int],
    save_interval: Optional[int],
    print_sample_every: Optional[int],
...
def train(
    ...
    beta: Optional[float] = typer.Option(None, "--beta", help="KL coefficient beta (policy divergence strength)."),
    group_size: Optional[int] = typer.Option(None, "--group-size", help="GRPO group size / num_generations."),
    task: Optional[str] = typer.Option(None, "--task", help="Task type preset for rewards/chat template (math, code, general, reasoning)."),
    rewards: Optional[str] = typer.Option(None, "--rewards", help="Comma-separated reward function names (overrides task preset)."),
    ...
    min_p: Optional[float] = typer.Option(None, "--min-p", help="Minimum probability threshold (min_p)."),
    max_new: Optional[int] = typer.Option(None, "--max-new", help="Max new tokens (completion length)."),
    max_prompt_len: Optional[int] = typer.Option(None, "--max-prompt-len", help="Max prompt token length."),
    max_seq_length: Optional[int] = typer.Option(None, "--max-seq-length", help="Override model's max sequence length."),
    prompt_length_pct: Optional[float] = typer.Option(None, "--prompt-length-pct", help="Percentile for prompt length filtering (0-1)."),
    eval_interval: Optional[int] = typer.Option(None, "--eval-interval", help="Evaluation / preview interval (steps)."),
    save_interval: Optional[int] = typer.Option(None, "--save-interval", help="Save interval (steps)."),
    print_sample_every: Optional[int] = typer.Option(None, "--print-sample-every", help="Print sample generations every N reward calls."),
    stop_sequences: Optional[str] = typer.Option(None, "--stop", help="Comma-separated stop sequences."),
    ...
    logging_steps: Optional[int] = typer.Option(None, "--logging-steps", help="Logging steps."),
    warmup_steps: Optional[int] = typer.Option(None, "--warmup-steps", help="Warmup steps."),
    ...
    save_total_limit: Optional[int] = typer.Option(None, "--save-total-limit", help="Max number of checkpoints to keep."),
    report_to: Optional[str] = typer.Option(None, "--report-to", help="Reporting target: tensorboard, wandb, none."),
    seed: Optional[int] = typer.Option(None, "--seed", help="Random seed."),
    # LoRA extras
    lora_dropout: Optional[float] = typer.Option(None, "--lora-dropout", help="LoRA dropout."),
    lora_targets: Optional[str] = typer.Option(None, "--lora-targets", help="Comma list of LoRA target module names."),
    # Device override
