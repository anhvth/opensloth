...
A unified command-line interface for preparing datasets for fine-tuning.
Supports various model families (Qwen, Gemma, Llama, etc.) and data sources.
"""
...
def list_presets() -> List[str]:
    ...
    preset_dir = get_project_root() / "prepare_dataset" / "presets" / "data"
...
def run_dataset_preparation(config: Dict[str, Any]) -> str:
    """Run dataset preparation using the existing prepare_dataset script."""
    ...
    try:
        # Detect model family for choosing preparer
        model_name = config.get("tok_name", "")
        family = detect_model_family(model_name) or "qwen"
        
        # Run the appropriate preparer script
        if family == "qwen":
            script_path = project_root / "prepare_dataset" / "prepare_qwen.py"
        elif family == "gemma":
            script_path = project_root / "prepare_dataset" / "prepare_gemma.py"
        else:
            script_path = project_root / "prepare_dataset" / "prepare_qwen.py"  # Default
        
        # Create a config file for the old-style preparer
        cmd = [
            sys.executable, str(script_path),
            "--tok_name", config.get("tok_name", ""),
            "--dataset_name", config.get("dataset_name", ""),
            "--num_samples", str(config.get("num_samples", -1)),
        ...
        if config.get("num_proc"):
            cmd.extend(["--num_proc", str(config["num_proc"])])
        if config.get("debug"):
            cmd.extend(["--debug", str(config["debug"])])
        if config.get("instruction_part"):
            cmd.extend(["--instruction_part", config["instruction_part"]])
        if config.get("response_part"):
            cmd.extend(["--response_part", config["response_part"]])
        if config.get("train_on_target_only"):
            cmd.append("--train_on_target_only")
...
Model Families:
  ...
  - Mistral: mistral, mixtral models
    ...
    parser.add_argument(
        ...
        type=str, choices=["qwen", "gemma", "llama"],
...
def main():
    ...
    try:
        ...
        # Run preparation
        output_dir = run_dataset_preparation(config)
