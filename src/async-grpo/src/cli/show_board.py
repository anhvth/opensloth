    ...
    if "result" in data:
        ...
        if "prompt_ids" in result:
            ...
            if hasattr(prompt_ids, "shape"):
                ...
                metrics["prompt_ids/seq_length"] = (
                    prompt_ids.shape[1] if len(prompt_ids.shape) > 1 else 0
                )
        ...
        if "completion_ids" in result:
            ...
            if hasattr(completion_ids, "shape"):
                ...
                metrics["completion_ids/seq_length"] = (
                    completion_ids.shape[1] if len(completion_ids.shape) > 1 else 0
                )
...
def write_metrics_to_tensorboard(
    writer: SummaryWriter, metrics: Dict[str, Any], step: int, timestamp: float
):
    ...
    categories = {
        ...
        "tokens": [
            ...
            "prompt_ids/seq_length",
            "completion_ids/batch_size",
            "completion_ids/seq_length",
        ],
        ...
        for metric_name in metric_names:
            if metric_name in metrics:
                ...
                if isinstance(value, (int, float)) and not (isinstance(value, bool)):
                    writer.add_scalar(
                        f"{category}/{metric_name.split('/')[-1]}", value, step
                    )
...
def main():
    ...
    all_files = AtomicFileOps.list_files(
        "worker/done/", "*_collected_consumed_*.pt", sort_by_mtime=True
    )
    ...
    if not result_files:
        ...
        print(
            "Looking for files matching pattern: worker/done/*_collected_consumed_*.pt (excluding .lock files)"
        )
        sys.exit(1)
