...
def main():
    ...
    parser.add_argument('--num_samples','-n', type=int, default=-1, help='Number of samples to process (use -1 for all)')
    parser.add_argument('--num_proc', '-wk',type=int, default=8, help='Number of processes for mapping')
    ...
    parser.add_argument('--debug', type=int, default=10, help='If >0, dump this many samples as HTML to output_dir/debug_samples.html')
    ...
    if args.num_samples > 0:
        ...
        # sample randomly 20
        all_messages = random.sample(all_messages, min(args.debug, len(all_messages)))

    def process_one(messages):
        text = tokenizer.apply_chat_template(messages['messages'], tokenize=False)
        if 'instruct-2507' in args.tok_name.lower():
    ...
    if args.debug > 0:
        ...
        debug_chat_dataloader_for_training_markdown(dataloader, tokenizer, n_example=args.debug)
