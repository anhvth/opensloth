...
set DEFAULT_MAX_SEQ_LENGTH 4096
set DEFAULT_NUM_PROC 8
set DEFAULT_GPUS 1

function usage
    echo "Usage: gen_llama_dataset.sh [OPTIONS] --output_dir OUTPUT_DIR"
    echo ""
    echo "Prepare dataset for Llama model fine-tuning"
    echo ""
    echo "Required:"
    ...
    echo "  ./gen_llama_dataset.sh --output_dir ./data/llama_finetome"
    echo ""
    echo "  # Custom model and dataset"
    echo "  ./gen_llama_dataset.sh --model unsloth/Llama-3.1-8B-Instruct --dataset my-dataset --output_dir ./data/custom"
    echo ""
    echo "  # Response-only training"
    echo "  ./gen_llama_dataset.sh --train_on_target_only --instruction_part '<|start_header_id|>user<|end_header_id|>' --response_part '<|start_header_id|>assistant<|end_header_id|>' --output_dir ./data/llama_response_only"
end
...
if $train_on_target_only
    if test -z "$instruction_part"
        set instruction_part "<|start_header_id|>user<|end_header_id|>\n\n"
    end
    if test -z "$response_part"
        set response_part "<|start_header_id|>assistant<|end_header_id|>\n\n"
    end
end
